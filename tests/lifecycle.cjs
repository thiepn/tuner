...
function setup(){
 ...
 class Audio{constructor(){this.sampleRate=48000}async resume(){}async close(){closed++}createMediaStreamSource(){return node()}createBiquadFilter(){return node()}createAnalyser(){return node()}}
 const c={console:{error(){}},navigator:{mediaDevices:{getUserMedia(){}}},window:{isSecureContext:true,AudioContext:Audio},DOMException,Float32Array,
...
(async()=>{
 ...
 t=setup();p=t.c.start();t.request.resolve(t.stream);assert.equal(await p,true);assert.equal(t.c.running,true);await t.c.stop();assert.equal(t.stops,1);assert.equal(t.closed,1);assert.equal(t.c.running,false);assert.equal(t.c.startBtn.disabled,false);
